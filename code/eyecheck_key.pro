PRO EYECHECK_KEY, template_path, spectra_path_prompt, spectra_path
CLOSE, 1 
...
permbuttons = ['ODD (O)','BAD (Z)','SMOOTH (S)','DONE (D)','BACK (B)','SAVE & QUIT (Q)']
...
;;                     IF cury LT 4.7 AND cury GE 2.5 AND curx GT 9500 THEN BEGIN
                    if strupcase(key) eq 'J' then begin
...
;;                     IF cury LT 2.5 AND cury GE 0.3 AND curx GT 9500 THEN BEGIN
                    if strupcase(key) eq 'K' then begin
